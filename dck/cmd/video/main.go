// Command video exports the complete game canvas and its own audio.
package main

import (
	"flag"
	"log"
	"time"

	"github.com/hajimehoshi/ebiten/v2"
	"github.com/olivierh59500/democonstructionkit/video"
	demo "github.com/olivierh59500/fr010/dck"
)

func main() {
	config := video.Config{Output: "go-fr010.mp4", PosterAt: 100 * time.Second, Title: "FR-010", Width: 640, Height: 480, FPS: 60, TPS: 60, SampleRate: 48000, Duration: 0}
	config.Flags(flag.CommandLine)
	flag.Parse()
	if err := video.Run(config, func() (ebiten.Game, error) {
		return demo.NewGame()
	}); err != nil {
		log.Fatal(err)
	}
}
